...
public:
  ...
  AABB();
  ~AABB();
  AABB(Vector2<T> _pos, T _xw, T _yw, const sLevelData *level_data);
...
//  virtual void ReportCollisionVsWorldVsRightSide();
  void Draw();
...
template<class T>
AABB<T>::AABB(Vector2<T> _pos, T _xw, T _yw, const sLevelData *level_data) :
...
void AABB<T>::CollideVsWorldBounds()
{
  ...
  x = m_level_data->m_world_min_width - (pos.x - xw);
  ...
  {
    x = pos.x + xw - m_level_data->m_world_max_width;
  ...
  y = m_level_data->m_world_min_height - (pos.y - yw);
  ...
  {
    y = pos.y + yw - m_level_data->m_world_max_height;
    if ( y > 0 )
    {
      ReportCollisionVsWorld(0, -y, 0, -1);
    }
