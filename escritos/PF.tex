...
\chapter{O Hardware do Nintendo DS\label{cap:hardds}}
...
O desenvolvimento no DS segue os mesmos moldes de desenvolvimento em consoles diversos: a Nintendo disponibiliza unidades especiais, de desenvolvimento, para os desenvolvedores licenciados. Essas unidades especiais são compostas de um console e uma unidade para debug, de modo que o jogo sendo desenvolvido possa ser enviado imediatamente para a unidade conectada e debugada num console que é quase igual ao original. A diferença dessa unidade para a unidade que é comercializada é principalmente no tamanho da memória principal, que passa de 4Mb para 8Mb.

No entanto, normalmente, desenvolvedores pequenos não tem acesso ao kit de desenvolvimento oficial, seja por causa dos altos custos para adquirir o hardware, seja pela dificuldade de adquirir as licenças. Nesse caso, cabe recorrer ao desenvolvimento não oficial, apelidado de Homebrew. A cadeia de ferramentas usada é chamada devkitArm. O devkitArm serve para compilar programas em C e C++ para consoles com processadores ARM. 
...
\chapter{A criação do jogo\label{cap:workflow}}

Um jogo necessita de algumas partes, como por exemplo, suas fases e cenários, seus ítens, inimigos e personagem principal. Tentamos cobrir essas partes em algumas ferramentas diferentes.

Em primeiro lugar, o usuário cria os cenários usando a BGTool, onde ele desenha os cenários em que o jogo vai se passar. Esses cenários ficam organizados logicamente em fases, e essas fases aparecerão encadeadas para o jogador, quando ele as exportar para o formato aceito pelo DS.

Depois, o usuário usa a SpriteTool para criar tudo relacionado aos sprites do jogo, sejam eles do personagem, dos inimigos ou de itens. Ela serve para adicionar os vários frames que compõe a animação, e encadear a animação.
